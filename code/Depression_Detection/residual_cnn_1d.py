...
class Residual_CNN_Model(pl.LightningModule):
    def __init__(self, output_class):
        ...
        self.cnn_block = nn.Sequential(nn.Conv1d(in_channels=5, out_channels=64*5, kernel_size=15, stride=1, padding=7, groups=5),
                                       nn.BatchNorm1d(num_features=64*5), 
                                       nn.ReLU(),
                                       nn.AvgPool1d(kernel_size=5, stride=2))
        
        self.res_block_1 = nn.ModuleList([Residual_Block(input_channels=64*1*5, output_channels=64*1*5, hidden_channels=64*1*5, stride=1) for i in range(4)])
        self.res_block_2 = nn.ModuleList([Residual_Block(input_channels=64*1*5, output_channels=64*2*5, hidden_channels=64*2*5, stride=1)] + [Residual_Block(input_channels=64*2*5, output_channels=64*2*5, hidden_channels=64*2*5, stride=1) for i in range(3)])
        self.res_block_3 = nn.ModuleList([Residual_Block(input_channels=64*2*5, output_channels=64*3*5, hidden_channels=64*3*5, stride=1)] + [Residual_Block(input_channels=64*3*5, output_channels=64*3*5, hidden_channels=64*3*5, stride=1) for i in range(5)])
        self.res_block_4 = nn.ModuleList([Residual_Block(input_channels=64*3*5, output_channels=64*4*5, hidden_channels=64*4*5, stride=1)] + [Residual_Block(input_channels=64*4*5, output_channels=64*4*5, hidden_channels=64*4*5, stride=1) for i in range(2)])
        
        self.linear = nn.Sequential(nn.Linear(1280, 1000),
        ...
        self.global_pool = nn.AvgPool1d(kernel_size=598)
...
if __name__ == '__main__':
    test = torch.rand((2, 5*1200))
    
    cnn_layer = Residual_CNN_Model(output_class=2)
    tmp = cnn_layer.forward(test.view(2, 5, 1200))
    print(tmp.shape)
# %%
