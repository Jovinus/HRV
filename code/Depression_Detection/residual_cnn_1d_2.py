...
class Residual_CNN_Model(pl.LightningModule):
    def __init__(self, output_class):
        ...
        self.cnn_block = nn.Sequential(nn.Conv1d(in_channels=5, out_channels=64*5, kernel_size=15, stride=1, padding=7, groups=5),
                                       ...
                                       nn.Conv1d(in_channels=64*5, out_channels=64, kernel_size=1, stride=1))
        ...
        self.res_block_2 = nn.ModuleList([Residual_Block(input_channels=64*1, output_channels=64*2, hidden_channels=64*2, stride=1)] + [Residual_Block(input_channels=64*2, output_channels=64*2, hidden_channels=64*2, stride=1) for i in range(3)])
        self.res_block_3 = nn.ModuleList([Residual_Block(input_channels=64*2, output_channels=64*3, hidden_channels=64*3, stride=1)] + [Residual_Block(input_channels=64*3, output_channels=64*3, hidden_channels=64*3, stride=1) for i in range(5)])
        self.res_block_4 = nn.ModuleList([Residual_Block(input_channels=64*3, output_channels=64*4, hidden_channels=64*4, stride=1)] + [Residual_Block(input_channels=64*4, output_channels=64*4, hidden_channels=64*4, stride=1) for i in range(2)])
