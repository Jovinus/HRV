...
if __name__ == '__main__':
    ...
    subject = df_orig[["file_nm", "session"]].query("session == [1, 2]").reset_index(drop=True)
    
    rkf = RepeatedStratifiedKFold(n_splits=10, n_repeats=1, random_state=1003)
    ...
    for cv_num, (train_id, test_id) in enumerate(rkf.split(subject, y=subject['session'])):
        ...
        valid_data = df_orig.query("file_nm.isin(@valid_id)", engine='python').reset_index(drop=True)
        test_data = df_orig.query("file_nm.isin(@test_id)", engine='python').reset_index(drop=True)
        ...
        logger = TensorBoardLogger("tb_logs", name="offline_2", version=cv_num)
        
        checkpoint_callback = ModelCheckpoint(monitor='val_loss',
                                      dirpath='check_point/offline_2_'+str(cv_num), 
    ...
    df_metric.reset_index(drop=True).to_csv("./offline_2_results.csv", index=False)
# %%
